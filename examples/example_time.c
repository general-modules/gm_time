...
int main(int argc, char *argv[])
{
    ...
    int ret = gm_time_get_compile_date_time(compile_date_time, sizeof(compile_date_time));
    if (ret == 0)
    {
    ...
    char timezone[64] = "";
    ret = gm_time_get_system_timezone(timezone, sizeof(timezone));
    if (ret == 0)
    {
        printf("timezone: %s\n", timezone);
    }
    ...
    ret = gm_time_get_date_time_str(date_time_str, sizeof(date_time_str), gap_flag);
    if (ret == 0)
    {
    ...
    ret = gm_time_get_uptime_ms(&uptime_ms);
    if (ret == 0)
    {
        printf("uptime(ms): %lu\n", uptime_ms);
    }
    ...
    ret = gm_time_get_local_time(&local_time);
    if (ret == 0)
    {
    ...
    ret = gm_time_get_current_timestamp_ms(&timestamp_ms);
    if (ret == 0)
    {
    ...
    gm_date_time_t local_time_base = {.tm_year = 2026,
                                      .tm_mon = 1,
                                      .tm_mday = 2,
                                      .tm_hour = 10,
                                      .tm_min = 20,
    ...
    ret = gm_time_convert_unix_to_local(&local_time, unix_timestamp_s);
    if (ret == 0)
    {
    ...
    unix_timestamp_s = 0;
    ret = gm_time_convert_local_to_unix(&unix_timestamp_s, &local_time);
    if (ret == 0)
    {
